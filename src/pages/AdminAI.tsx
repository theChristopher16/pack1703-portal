  ...
  const handleComprehensiveEventCreation = async (prompt: string) => {
    try {
      ...
      const testMessage: ChatMessage = {
        id: Date.now().toString(),
        content: `🎯 **Comprehensive Event Preview**\n\nI've analyzed your request and here's what I would create:\n\n**Event:** ${testResult.eventData.title}\n**Dates:** ${testResult.eventData.startDate} to ${testResult.eventData.endDate}\n**Location:** ${testResult.eventData.location}\n**Address:** ${testResult.eventData.address.includes('Unable to find') ? '⚠️ ' + testResult.eventData.address : testResult.eventData.address}\n**Phone:** ${testResult.eventData.phone.includes('Unable to find') ? '⚠️ ' + testResult.eventData.phone : testResult.eventData.phone}\n\n**Safety Information:**\n🏥 **Nearest Medical Facility:** ${testResult.eventData.medicalFacility.name.includes('Unable to find') ? '⚠️ ' + testResult.eventData.medicalFacility.name : testResult.eventData.medicalFacility.name} (${testResult.eventData.medicalFacility.distance})\n📞 **Emergency Contact:** ${testResult.eventData.medicalFacility.phone.includes('Unable to find') ? '⚠️ ' + testResult.eventData.medicalFacility.phone : testResult.eventData.medicalFacility.phone}\n\n**Family Safety:**\n⚠️ **Allergies to Watch:** ${testResult.eventData.allergies.join(', ')}\n♿ **Accessibility:** ${testResult.eventData.accessibility}\n🌤️ **Weather:** ${testResult.eventData.weatherConsiderations}\n\n**5-Day Weather Forecast:**\n${testResult.weatherForecast ? testResult.weatherForecast.dailyForecasts.map((day: any) => `📅 **${day.dayName} (${day.date})**: ${day.minTemperature}°F - ${day.maxTemperature}°F, ${day.conditions} (${day.description})`).join('\n') : '⚠️ Weather forecast not available - coordinates needed'}\n\n**Timing:**\n⏰ **Check-in:** ${testResult.eventData.checkInTime}\n🕐 **Earliest Arrival:** ${testResult.eventData.earliestArrival}\n\n**Packing List:**\n${testResult.packingList.map((item: string) => `• ${item}`).join('\n')}\n\n**Family Notes:**\n${testResult.familyNotes}\n\n**Duplicate Check:**\n${testResult.duplicateCheck.shouldCreateNew ? '✅ Creating new location' : '📍 Using existing location'}\n${testResult.duplicateCheck.reason}\n\n${testResult.eventData.phone.includes('Unable to find') || testResult.eventData.address.includes('Unable to find') || testResult.eventData.medicalFacility.name.includes('Unable to find') ? '⚠️ **Note:** Some information could not be found automatically. You can edit these fields after creating the event.' : ''}\n\nWould you like me to create this event? Type "yes" to proceed or "no" to cancel.`,
        sender: 'ai',
        timestamp: new Date(),
        type: 'event-preview',
        data: testResult
